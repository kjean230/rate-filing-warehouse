...
- Orchestration: decided at Phase 6 — a stdlib DAG runner (`rfp-run`,
  `pipeline/orchestrate/`), ADR 0020; Dagster / Airflow / Prefect
  evaluated and rejected. Not a scheduler, not retries, not a platform.
...
pipeline/
  ingest/  extract/  validate/  cdc/  orchestrate/
config/
...
| 6 Orchestration ✅ | DAG: ingest → detect → [extract `--filing` per stale filing] → re-detect → validate → load → dbt build — `rfp-run`, a stdlib DAG runner (ADR 0020) | One bad filing fails in isolation. **Gate passed 2026-08-22, awaiting approval** — stated as a property and broken on purpose (`tests/orchestrate/test_isolation.py`): B's extract fails three ways, A and C still run in order, the record names B, the re-detect blocks the warehouse, the next run retries only B; the sibling test shows the Airflow-default `all_success` short-circuit leaves C un-extracted. Real `rfp-run --offline` end to end over the fixture tree (`-m warehouse`) and over the real corpus twice — offline (30 current, validate skipped (current), 11,775 raw rows, `dbt build` 148/148, exit 0) and online (30 conditional GETs → 30 × 304, detect exit 0, 11,805 rows, 148/148, exit 0, $0) — record under `data/orchestration/`. "One bad filing fails in isolation" is a *sequencing / isolation* property over nodes already isolated (ADRs 0004/0006/0009/0012) — not a scheduler, not retries, not a platform; no live extract was run. See ADR 0020. |
